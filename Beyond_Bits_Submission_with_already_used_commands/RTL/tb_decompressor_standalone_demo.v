...
// into rle_decompressor and writes the re-inflated bits to recovered_bits.txt.
//
...
module tb_decompressor;
    ...
    initial begin
        // INPUT_FILE selectable at compile/run time; default compressed.txt
        fd_in  = $fopen("compressed.txt", "r");
        fd_out = $fopen("recovered_bits.txt", "w");

        if (fd_in == 0) begin
            $display("ERROR: compressed.txt could not be found!");
            $finish;
        end
        ...
        $fclose(fd_in);
        $fclose(fd_out);
        $display("SUCCESS: Decompression done. Check recovered_bits.txt");
        $finish;
    end
endmodule
